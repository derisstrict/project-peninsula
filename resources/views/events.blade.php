...
<p class="text-5xl text-center font-semibold">{!! __('events.title', ['accent' => '<span class="text-color-accent"> '. __('events.title_accent') .' </span>']) !!}
</p>
...
<a x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" href="/events/events-detail" class="flex bg-color-accent/5 p-8 rounded-lg mt-10 gap-5 w-full items-center hover:bg-color-accent/10 transition-colors">
    ...
    <div class="flex flex-col gap-3">
        ...
        <p class="text-3xl font-semibold">IDR <span class="text-color-accent">75.000</span></p>
    </div>
...
<div class="grid grid-cols-1 mt-10 gap-4 lg:grid-cols-3 md:grid-cols-2">
    @foreach ($events as $event)
        <x-events-card href="/events/{{ $event->id }}/{{ $event->slug }}" date="{{ Carbon::parse($event->tanggal_mulai)->translatedFormat('d F Y') }} - {{ Carbon::parse($event->tanggal_selesai)->translatedFormat('d F Y') }}" image="{{ asset('storage/' . $event->gambar_event) }}" price="{{ number_format($event->harga_tiket, 0, '.', '.') }}" alt="{{ $event->alt_gambar }}">{{ $event->judul_event }}</x-events-card> 
    @endforeach
</div>
@endsection
