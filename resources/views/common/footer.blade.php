
<footer class=" py-8 border-t border-lime-600 dark:border-lime-700">
<div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 px-6 lg:gap-8 items-start">
    <div>
        <p class="text-2xl font-semibold leading-tight">The<br>
            <span class="text-lime-600 dark:text-lime-400">Peninsula Island</span>
        </p>
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">
            Lorem ipsum dolor sit amet.
        </p>
    </div>

    <div>
        <h4 class="font-semibold text-lime-600 dark:text-lime-300 mb-1">Social Media</h4>
        <ul class="flex flex-col gap-2 text-sm">
            <li class="flex items-center gap-2">
                <img src="/img/dark_instagram.png" alt="Instagram" class="w-5 h-5 block dark:hidden">
                <img src="/img/instagram.png" alt="Instagram" class="w-5 h-5 hidden dark:block">
                <a href="#" class="text-gray-700 dark:text-gray-300 hover:text-lime-600 dark:hover:text-lime-400 transition-colors">Instagram</a>
            </li>

            <li class="flex items-center gap-2">
                <img src="/img/dark_tiktok.png" alt="TikTok" class="w-5 h-5 block dark:hidden">
                <img src="/img/tiktok.png" alt="TikTok" class="w-5 h-5 hidden dark:block">
                <a href="#" class="text-gray-700 dark:text-gray-300 hover:text-lime-600 dark:hover:text-lime-400 transition-colors">TikTok</a>
            </li>

            <li class="flex items-center gap-2">
                <img src="/img/dark_email.png" alt="Email" class="w-5 h-5 block dark:hidden">
                <img src="/img/email.png" alt="Email" class="w-5 h-5 hidden dark:block">
                <a href="mailto:theemail@gmail.com" class="text-gray-700 dark:text-gray-300 hover:text-lime-600 dark:hover:text-lime-400 transition-colors">theemail@gmail.com</a>

            </li>
        </ul>
    </div>

    <div>
        <h4 class="font-semibold text-green-700 dark:text-lime-300 mb-1">Menu</h4>
        <ul class="flex flex-col gap-4 text-sm">
            <li class="flex items-center gap-2">
                <a href="#" class="hover:text-green-600 dark:hover:text-lime-400 transition-colors">Home</a>
            </li>
            <li class="flex items-center gap-2">
                <a href="#" class="hover:text-green-600 dark:hover:text-lime-400 transition-colors">Event</a>
            </li>
            <li class="flex items-center gap-2">
                <a href="#" class="hover:text-green-600 dark:hover:text-lime-400 transition-colors">Blogs</a>
            </li>
        </ul>
    </div>

    <div>
        <div class="flex items-center gap-3 mb-2">
        <h4 class="font-semibold text-gray-700 dark:text-gray-100">Language:</h4>
        <div class="relative" x-data="{ dropLang: false }">
                <button @click="dropLang = !dropLang" @click.outside="dropLang = false" id="dropdownLanguage" aria-expanded="false" class="relative flex gap-2 bg-light-primary/10 p-2 px-3 rounded-xl items-center cursor-pointer hover:bg-light-primary/15 dark:bg-dark-primary/10 dark:hover:bg-dark-primary/15 transition" type="button">
                    <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M14.6921 5H9.30807C8.15914 5.00635 7.0598 5.46885 6.25189 6.28576C5.44398 7.10268 4.99368 8.20708 5.00007 9.356V14.644C4.99368 15.7929 5.44398 16.8973 6.25189 17.7142C7.0598 18.5311 8.15914 18.9937 9.30807 19H14.6921C15.841 18.9937 16.9403 18.5311 17.7482 17.7142C18.5562 16.8973 19.0064 15.7929 19.0001 14.644V9.356C19.0064 8.20708 18.5562 7.10268 17.7482 6.28576C16.9403 5.46885 15.841 5.00635 14.6921 5Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M8.00012 9C7.58591 9 7.25012 9.33579 7.25012 9.75C7.25012 10.1642 7.58591 10.5 8.00012 10.5V9ZM12.0001 10.5C12.4143 10.5 12.7501 10.1642 12.7501 9.75C12.7501 9.33579 12.4143 9 12.0001 9V10.5ZM11.2501 9.75C11.2501 10.1642 11.5859 10.5 12.0001 10.5C12.4143 10.5 12.7501 10.1642 12.7501 9.75H11.2501ZM12.7501 8C12.7501 7.58579 12.4143 7.25 12.0001 7.25C11.5859 7.25 11.2501 7.58579 11.2501 8H12.7501ZM12.0001 9C11.5859 9 11.2501 9.33579 11.2501 9.75C11.2501 10.1642 11.5859 10.5 12.0001 10.5V9ZM15.5001 10.5C15.9143 10.5 16.2501 10.1642 16.2501 9.75C16.2501 9.33579 15.9143 9 15.5001 9V10.5ZM15.5001 9C15.0859 9 14.7501 9.33579 14.7501 9.75C14.7501 10.1642 15.0859 10.5 15.5001 10.5V9ZM16.0001 10.5C16.4143 10.5 16.7501 10.1642 16.7501 9.75C16.7501 9.33579 16.4143 9 16.0001 9V10.5ZM16.1138 10.1811C16.3519 9.84222 16.2702 9.37443 15.9313 9.13631C15.5923 8.8982 15.1246 8.97992 14.8864 9.31885L16.1138 10.1811ZM11.2737 13.2783C10.9579 13.5464 10.9193 14.0197 11.1874 14.3354C11.4555 14.6512 11.9288 14.6898 12.2445 14.4217L11.2737 13.2783ZM9.29973 14.9003C8.96852 15.149 8.90167 15.6192 9.15041 15.9504C9.39916 16.2816 9.8693 16.3485 10.2005 16.0997L9.29973 14.9003ZM12.2569 14.407C12.5667 14.1321 12.595 13.6581 12.3201 13.3483C12.0453 13.0384 11.5712 13.0101 11.2614 13.285L12.2569 14.407ZM11.1691 14.3091C11.4249 14.6349 11.8963 14.6917 12.2222 14.436C12.548 14.1802 12.6048 13.7088 12.3491 13.3829L11.1691 14.3091ZM11.186 11.4467C11.0185 11.0678 10.5756 10.8966 10.1968 11.0641C9.81796 11.2316 9.64667 11.6745 9.8142 12.0533L11.186 11.4467ZM12.3609 13.4024C12.1137 13.07 11.6439 13.001 11.3115 13.2482C10.9792 13.4954 10.9101 13.9652 11.1573 14.2976L12.3609 13.4024ZM13.8953 16.6608C14.2602 16.8567 14.7149 16.7198 14.9109 16.3548C15.1068 15.9899 14.9699 15.5352 14.605 15.3392L13.8953 16.6608ZM8.00012 10.5H12.0001V9H8.00012V10.5ZM12.7501 9.75V8H11.2501V9.75H12.7501ZM12.0001 10.5H15.5001V9H12.0001V10.5ZM15.5001 10.5H16.0001V9H15.5001V10.5ZM14.8864 9.31885C13.8552 10.7867 12.6412 12.1172 11.2737 13.2783L12.2445 14.4217C13.7091 13.1782 15.0093 11.7532 16.1138 10.1811L14.8864 9.31885ZM10.2005 16.0997C10.7113 15.7161 11.4531 15.1201 12.2569 14.407L11.2614 13.285C10.4871 13.9719 9.77692 14.5419 9.29973 14.9003L10.2005 16.0997ZM12.3491 13.3829C11.8824 12.7884 11.4917 12.1379 11.186 11.4467L9.8142 12.0533C10.1703 12.8586 10.6255 13.6164 11.1691 14.3091L12.3491 13.3829ZM11.1573 14.2976C11.8855 15.2767 12.8203 16.0835 13.8953 16.6608L14.605 15.3392C13.7239 14.8661 12.9578 14.2048 12.3609 13.4024L11.1573 14.2976Z"/>
                    </svg>
                    EN
                    <svg class="rotate-90" width="16px" height="16px" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M7.82054 20.7313C8.21107 21.1218 8.84423 21.1218 9.23476 20.7313L15.8792 14.0868C17.0505 12.9155 17.0508 11.0167 15.88 9.84497L9.3097 3.26958C8.91918 2.87905 8.28601 2.87905 7.89549 3.26958C7.50497 3.6601 7.50497 4.29327 7.89549 4.68379L14.4675 11.2558C14.8581 11.6464 14.8581 12.2795 14.4675 12.67L7.82054 19.317C7.43002 19.7076 7.43002 20.3407 7.82054 20.7313Z"/>
                    </svg>
                </button>
                <div x-show="dropLang" id="lang-menu" class="absolute z-10 right-50 left-0 mt-4 h-10 bg-light-primary/15 py-4 w-55 h-fit rounded-xl dark:bg-dark-primary/15">
                    <ul class="flex flex-col">
                        <li>
                            <a href="" class="block p-2 hover:bg-light-primary/25 dark:hover:bg-dark-primary/25">English</a>
                        </li>
                        <li>
                            <a href="" class="block p-2 hover:bg-light-primary/25 dark:hover:bg-dark-primary/25">Bahasa Indonesia</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>

        <p class="text-sm text-gray-600 dark:text-gray-400">
            If you’ve encountered a broken public facility let us know!<br>
            <a href="#" class="font-semibold text-lime-600 dark:text-lime-300 hover:underline">Make a report</a>
        </p>
    </div>

</div>
</footer>

